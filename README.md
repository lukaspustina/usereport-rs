...
| Signal | Source |
|--------|--------|
| `cpu.usr_pct`, `cpu.iowait_pct`, `cpu.run_queue` | `/proc/stat` |
...
| Cross-signal comparison | `cpu.run_queue > host.cpu_count` |
...
> **Note:** The `Formula/` directory is generated by cargo-dist on each release and is not committed to the main repository branch.
...
```sh
usereport --exit-on info   # exit 1 if any Info, Warn, or Crit finding fires
...
$ usereport --exit-on crit; echo "exit: $?"
exit: 2   # one or more Crit findings fired
```
...
```toml
[dependencies]
usereport-rs = { version = "0.2", features = ["bin"] }
```

> **Note:** `features = ["bin"]` is required to use `Config`, `Analysis`, and the CLI types.
