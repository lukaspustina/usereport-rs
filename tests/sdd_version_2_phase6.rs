...
fn ac_phase6_4_redact_replaces_hostname_and_ip() {
    ...
    let output = LlmOutput {
        ...
        findings: vec![],
        checked_ok: vec![],
        raw_excerpts: vec![usereport::llm::LlmExcerpt {
            command: "dmesg".to_string(),
            output: "OOM kill on 192.168.1.100".to_string(),
        }],
    };
    ...
    assert_ne!(
        redacted.host.hostname, "prod-server.internal",
        "hostname must be hashed"
    );
...
const FIXTURE_THERMAL_INPUT: &str = include_str!("../skills/usereport-analyze/fixtures/thermal-throttle/input.json");
...
const FIXTURE_THERMAL_REF: &str = include_str!("../skills/usereport-analyze/fixtures/thermal-throttle/reference.md");
...
fn ac_phase6_8_fixture_input_files_are_valid_json_with_schema_version() {
    for (name, content) in [
        ...
        let v: serde_json::Value = serde_json::from_str(content)
            .unwrap_or_else(|e| panic!("fixture '{}/input.json' invalid JSON: {}", name, e));
        assert_eq!(
