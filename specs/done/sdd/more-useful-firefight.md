...
- **`SampleStats.trend`:** trend lives at `signal.stats.as_ref().map(|s| s.trend)` (`src/signal.rs:62`) — `Signal` has no `.trend` field
...
- **Phase ordering:** Phase 3 depends on the `RuleEngine::run` signature change introduced in Phase 2. Implement Phase 2 before Phase 3. Phases 4 and 5 are independent of each other and of Phase 3. Both Phase 2 and Phase 5 modify `run_diagnostics`; Phase 2 owns the signature change — Phase 5 extends it further.
- **`Config::validate` already exists** at `src/cli/config.rs:134` — Phase 5 extends it; the call site at `cli/mod.rs:311` is unchanged.
- **`RuleEngine::run` call sites** to update in Phase 2: `tests/sdd_version_2_phase1.rs` (5 calls), `tests/sdd_version_2_phase4.rs` (1 call), `tests/sdd_version_2_phase5.rs` (3 calls), `tests/sdd_version_2_phase7.rs` (3 calls), `tests/sdd_version_2_phase8.rs` (3 calls), `src/rule/mod.rs` inline test module (1 call, ~line 739). All must pass `&HashMap::new()` and destructure the tuple. Update in-place; do not create new files.
- **`Severity::rank`** at `src/finding.rs:41` is currently `fn rank(self) -> u8` (private, value receiver) — Phase 2 must change it to `pub fn rank(self) -> u8`.
...
│  1. Config::from_str → Config::validate (already called)            │
│  2. Build source_map: HashMap<signal_id, Vec<command_name>>          │
│     by hard-coding signal_id → collector.source_commands() mappings  │
└──────────────────────────────────────────────────────────────────────┘
                   │
                   ▼
┌──────────────────────────────────────────────────────────────────────┐
...
13. The system shall include the notice `"findings may be incomplete"` (exact lowercase string, no emoji) in the Coverage Gaps section when any commands were skipped.
14. The system shall populate `AnalysisReport.checked_ok` with the IDs of signals that appear in `evidence_ids` of any rule that was evaluated but did not fire. A rule is "evaluated" iff `rule.when.evaluate(...)` is called regardless of signal presence. A signal ID enters `checked_ok` when it appears in `evidence_ids` of a rule whose predicate returned `false`. Return as a sorted `Vec<String>`.
...
22. In HTML output, command sections that contributed evidence to any finding shall be rendered inline before all other command sections, with a badge "Evidence for: {finding-id}". When one command is evidence for multiple findings, it appears once with a comma-separated badge listing all finding IDs. Evidence commands are ordered by their position in `command_results[0]` (first repetition; subsequent repetitions are ignored for ordering).
...
25. In the suggest rendering, when a suggest string exactly equals the `.command()` field of a `CommandResult` (via the existing `.command()` accessor, excluding `SkippedMissing` results), the system shall render `<a href="#cmd-{name}">(see: {command.title} ↓)</a>` instead of the raw string in HTML, and append ` (see above)` in Markdown. Matching uses exact string equality.
...
27. Each Vital Signs resource line shall show the relevant signal values (see Data Models for field→signal_id mapping), a trend arrow (`↑` Rising, `↓` Falling, `→` Flat) when `signal.stats.as_ref().map(|s| s.trend)` is present on any matching signal, and a severity badge (`WARN`, `CRIT`) when a finding covers that resource. When Crit and Warn both appear, the line shows `CRIT` (lowest `rank()` wins). A resource line shows `[not profiled]` when all its signal fields are `None`.
...
36. When extraction finds no matching lines and `aggregate != Count`, the system shall emit nothing and log `log::warn!("extract: no matches for command={name} pattern={pattern}")`.
37. When extraction finds a non-numeric `val` capture, the system shall log `log::warn!("extract: non-numeric capture command={name} pattern={pattern} value={val}")`, then continue to the next line.
38. The system shall extend `Config::validate` (at `src/cli/config.rs:134`) to validate that `extract.pattern` is a valid regex and (unless `aggregate = Count`) contains the substring `(?P<val>`. Validation errors use a new `Error::InvalidExtractPattern { command: String, pattern: String, reason: String }` variant added to the existing `cli/config` error type. Do not change the `Config::validate` call site at `cli/mod.rs:311`.
39. The system shall extend `usereport explain` to accept a command name: load the config using the same path resolution as `generate_report` (platform default when `--config` is absent), pass it into `run_explain` as `config: &Config`, then look up the command by name. If found, render its `title`, `description`, `what_to_look_for` (if set), `extract` entries (pattern, signal_id, unit, aggregate), and `links`. If no command matches, fall back to existing rule/signal lookup. When the config file is absent, skip command lookup and fall back to rule/signal lookup only.
40. Multiple `extract` entries on the same command sharing the same `signal_id` are permitted; each entry produces an independent `Signal` emission (both signals are emitted and the rule engine sees both).
...
| File | Change |
|------|--------|
| `src/command.rs` | Add `install_hint`, `what_to_look_for`, `use_dimension`, `extract` fields (all `pub(crate)`); add accessor and builder methods; add `UseDimension`, `UseResource`, `UseAspect`, `CommandExtract`, `Aggregate` types |
| `src/cli/config.rs` | Add `ProfileFollowup` type; add `followup` to `Profile`; add `Error::InvalidExtractPattern` variant; extend `Config::validate` with private `validate_extract_patterns` helper |
...
| `src/collector/disk.rs` | Implement `source_commands()` (both platforms) |
| `src/collector/network.rs` | Implement `source_commands()` (platform-gated) |
| `src/collector/host.rs` | Implement `source_commands()` |
| `src/finding.rs` | Add `source_commands: Vec<String>` to `Evidence`; change `fn rank(self) -> u8` to `pub fn rank(self) -> u8` |
| `src/rule/mod.rs` | Change `RuleEngine::run` signature to accept `source_map` and return `(Vec<Finding>, Vec<String>)`; `pub fn rules(&self) -> &[Rule]` already exists |
| `src/analysis.rs` | Change `run_diagnostics` to `fn run_diagnostics(&self, command_results: &[CommandResult]) -> (Vec<Signal>, Vec<Finding>, Vec<String>)`; update `Analysis::run` to pass command results and populate `AnalysisReport.checked_ok`; add `pub(crate) vital_signs: VitalSigns` and `pub(crate) use_coverage: Vec<UseCoverageEntry>` fields to `AnalysisReport` with `#[serde(skip)]` |
| `src/extract.rs` | New file: `pub fn extract_signals(command_name: &str, stdout: &str, extracts: &[CommandExtract]) -> Vec<Signal>` |
| `src/cli/mod.rs` | Compute `signal_thresholds`, `use_coverage`, `VitalSigns`; build `source_map`; extend `run_explain` to accept `config: &Config`; pass all computed values to template context |
...
| `tests/sdd_more_useful_firefight_p{N}_c{N}_{description}.rs` | New test files per phase (naming convention) |

### Existing signatures for reference

- `RuleEngine::run` current: `pub fn run(&self, signals: &[Signal], ctx: &CollectCtx) -> Vec<Finding>`
- `run_diagnostics` current (in `src/analysis.rs:147`): `fn run_diagnostics(&self) -> (Vec<Signal>, Vec<Finding>)`
- `run_explain` current location: `src/cli/mod.rs:629` — `fn run_explain(id: &str) -> miette::Result<()>`
- `Config::validate` current: `pub fn validate(&self) -> Result<()>` at `src/cli/config.rs:134`; called at `cli/mod.rs:311` with `.into_diagnostic()?`
- `CollectCtx`: defined in the codebase; unchanged by this SDD — pass through as-is
- `RuleEngine::rules()`: `pub fn rules(&self) -> &[Rule]` at `src/rule/mod.rs:461` — already exists
...
pub struct CommandExtract {
    ...
    pub unit: Unit,             // uses existing Unit enum from src/signal.rs
                                // TOML "ms" → MillisPerOp, "bytes" → BytesPerSec, "pct" → Pct, "count" → Count
...
// Additions to existing Command struct fields (pub(crate), matching existing pattern):
...
//
// Accessor methods:
//   pub(crate) fn install_hint(&self) -> Option<&str>
//   pub(crate) fn what_to_look_for(&self) -> Option<&str>
//   pub(crate) fn use_dimension(&self) -> Option<&UseDimension>
//   pub(crate) fn extract(&self) -> &[CommandExtract]
//
// Builder methods (T: Into<String>, infallible — follow with_title pattern, not with_description):
//   pub(crate) fn with_install_hint(mut self, v: impl Into<String>) -> Self
//   pub(crate) fn with_what_to_look_for(mut self, v: impl Into<String>) -> Self
//   pub(crate) fn with_use_dimension(mut self, v: UseDimension) -> Self
...
// Addition to existing Profile struct:
// #[serde(default)]
// pub followup: Vec<ProfileFollowup>,


// src/finding.rs — modified Evidence
// Change fn rank(self) -> u8 to pub fn rank(self) -> u8 at src/finding.rs:41
//
// Add to Evidence struct:
// #[serde(default, skip_serializing_if = "Vec::is_empty")]
// pub source_commands: Vec<String>,   // empty until populated by rule engine (Phase 3)
// Update all Evidence construction sites to include source_commands: Vec::new()
...
// that returned false. Pass &HashMap::new() in Phase 2 before source_map is built (Phase 3).
// Walk the Predicate tree to populate source_commands: match on
// Predicate::Cmp { path, .. } → look up source_map.get(&path.join(".")) and clone into evidence.source_commands.


// src/analysis.rs — modified run_diagnostics
// Before: fn run_diagnostics(&self) -> (Vec<Signal>, Vec<Finding>)
// After:  fn run_diagnostics(&self, command_results: &[CommandResult]) -> (Vec<Signal>, Vec<Finding>, Vec<String>)
// The Vec<String> is checked_ok, threaded out to Analysis::run → AnalysisReport.checked_ok
// The command_results slice is the flattened first repetition from Analysis::run.
//
// Addition to AnalysisReport struct (after existing fields):
// #[serde(skip)]
// pub(crate) vital_signs: VitalSigns,
// #[serde(skip)]
// pub(crate) use_coverage: Vec<UseCoverageEntry>,
// Note: #[serde(skip)] keeps usereport diff backward-compatible.
// These fields are computed in generate_report and assigned directly.
...
// Key = signal_id. Built by walking RuleEngine.rules() predicates.
// Walk the Predicate tree: match on Predicate::Cmp { path, op, rhs: Rhs::Value(Value::Number(n)) }
//   to extract signal_id = path.join("."), op string, and threshold f64.
// Skip And, Or, and non-numeric Rhs variants silently.
...
// Severity::rank is made pub in Phase 2 (src/finding.rs:41).


// VitalSigns — computed in generate_report, stored on AnalysisReport.vital_signs
// Trend accessed via: signal.stats.as_ref().map(|s| s.trend)
// When multiple cpu.* signals have stats, use the first one found.
// Trend is NOT a field on Signal directly.
...
pub struct CpuLine {
    ...
    pub trend: Option<Trend>,           // from signal.stats.as_ref().map(|s| s.trend) for any cpu.* signal
    pub severity: Option<Severity>,     // highest severity (lowest rank()) of findings whose evidence signal_id starts with "cpu."
}
...
pub struct MemLine {
    ...
    // No trend field — only CPU has trend in current collectors; asymmetry is intentional.
}
...
// Computed in generate_report (which has both Config and AnalysisReport), then assigned to
// AnalysisReport.use_coverage directly. Config is available in generate_report already.
```
...
| `extract[].unit` | `String` | Yes | — | One of: `"pct"` → `Pct`, `"count"` → `Count`, `"ms"` → `MillisPerOp`, `"bytes"` → `BytesPerSec` |
...
Hard-code the signal_id → command_names mapping in `generate_report`: for each collector, for each signal ID that collector produces (derived from `evidence_ids` in `src/rule/builtin.rs`), insert `(signal_id, collector.source_commands().to_vec())` into `source_map: HashMap<String, Vec<String>>`. Collectors do not expose a `signal_ids()` method — the mapping is a hard-coded table in `generate_report`.
...
|---------|---------|-----------|--------------|
| Invalid regex in `extract.pattern` | `Config::validate` (extended in Phase 5 via `validate_extract_patterns` helper) | Return `Err(Error::InvalidExtractPattern { command, pattern, reason })` | `"command '{name}': extract pattern '{pattern}' is not a valid regex: {err}"` |
| Missing named group `(?P<val>...)` in `extract.pattern` (non-Count aggregate) | `Config::validate` | Same error variant | `"command '{name}': extract pattern '{pattern}' must contain a named group (?P<val>...)"` |
...
- `src/command.rs`:
  - Add `#[serde(skip_serializing_if = "Option::is_none")] pub(crate) install_hint: Option<String>`.
  - Add `#[serde(skip_serializing_if = "Option::is_none")] pub(crate) what_to_look_for: Option<String>`.
  - Add accessor methods `pub(crate) fn install_hint(&self) -> Option<&str>` and `pub(crate) fn what_to_look_for(&self) -> Option<&str>`.
  - Add builder methods `pub(crate) fn with_install_hint(mut self, v: impl Into<String>) -> Self` and `pub(crate) fn with_what_to_look_for(mut self, v: impl Into<String>) -> Self`, following the `with_title` pattern (`T: Into<String>`, infallible, no `Option` wrapping).
- `contrib/linux.conf`:
  ...
  - Add accurate description to `dmesg`.
  ...
  - Update `slabinfo` description: describe kernel slab allocator cache statistics.
...
- `src/finding.rs`:
  - Change `fn rank(self) -> u8` to `pub fn rank(self) -> u8` at line 41.
- `src/rule/mod.rs`:
  ...
  - `checked_ok` (second return value): sorted `Vec<String>` of signal IDs that appear in `evidence_ids` of any evaluated rule that returned `false` when its predicate was called. A rule is "evaluated" iff `rule.when.evaluate(...)` is called; absent signals cause the predicate to return `false` which still counts.
  - In Phase 2, the `source_map` parameter is accepted but not used for populating `source_commands` — that is wired in Phase 3. Pass `&HashMap::new()` at all call sites in Phase 2.
- `src/analysis.rs`:
  - Change `run_diagnostics` to: `fn run_diagnostics(&self, command_results: &[CommandResult]) -> (Vec<Signal>, Vec<Finding>, Vec<String>)`.
  - In `Analysis::run`, pass the flattened first repetition of `command_results` to `run_diagnostics`. Destructure the triple return and set `AnalysisReport.checked_ok`.
  - Pass `&HashMap::new()` as `source_map` to `RuleEngine::run` in Phase 2; Phase 3 replaces this.
- Update all `engine.run` call sites to accept the new signature and destructure the tuple:
  - `tests/sdd_version_2_phase1.rs` (5 calls) — update in-place
  - `tests/sdd_version_2_phase4.rs` (1 call) — update in-place
  - `tests/sdd_version_2_phase5.rs` (3 calls) — update in-place
  - `tests/sdd_version_2_phase7.rs` (3 calls) — update in-place
  - `tests/sdd_version_2_phase8.rs` (3 calls) — update in-place
  - `src/rule/mod.rs` inline test module (~line 739) — update in-place
- `src/cli/mod.rs` (`generate_report`):
  - Build `signal_thresholds: HashMap<String, ThresholdInfo>` by walking `rule_engine.rules()`.
  - Walk each rule's `when: Predicate` tree: match `Predicate::Cmp { path, op, rhs: Rhs::Value(Value::Number(n)) }` → `signal_id = path.join(".")`, extract op string and threshold f64. Skip `And`, `Or`, and non-numeric `Rhs` variants silently.
  - For each signal_id, keep only the `ThresholdInfo` with the lowest `Severity::rank()` (most severe).
  - Pass `signal_thresholds` into the template context.
- `contrib/html.j2` and `contrib/markdown.j2`:
  - Add **Coverage Gaps** section before Findings: iterate `command_results` for `SkippedMissing` variants. Show command name, binary, and `install_hint` when present. Include the notice `"findings may be incomplete"` (exact string) when list is non-empty.
  - Add **Healthy** section after Findings when `checked_ok` is non-empty: list signal IDs.
  - Add "vs. threshold" column to Signals table: render `"{signal.observed_value} / {threshold.value} {threshold.severity}"` if `signal_thresholds[signal.id]` exists, else blank.
...
- The Coverage Gaps section contains the exact string `"findings may be incomplete"` when any command was skipped.
- A report with no issues shows a "Healthy" section listing signals checked OK.
- The Signals table renders a "vs. threshold" column where populated rows contain the numeric threshold and severity label.
...
**C3** GIVEN `sar_cpu` is skipped WHEN the Coverage Gaps section renders THEN the output contains the string `"findings may be incomplete"`.
...
- `src/cli/mod.rs` (`generate_report`):
  - Build `source_map: HashMap<String, Vec<String>>` by hard-coding signal_id → command_names per collector (derived from `evidence_ids` in `src/rule/builtin.rs`). For each collector, for each signal ID it produces, insert `(signal_id, collector.source_commands().to_vec())`.
...
- `contrib/html.j2`:
  ...
  - After the Findings section, render promoted **Evidence Commands** sub-section: for commands referenced in any finding's evidence, render in `command_results[0]` insertion order. When one command is evidence for multiple findings, render it once with a badge listing all finding IDs (comma-separated): `"Evidence for: {id1}, {id2}"`.
  - Wrap all remaining non-evidence command sections in `<details><summary>Full output (N commands)</summary>`.
  - In suggest rendering: when a suggest string exactly equals a `CommandResult`'s `.command()` value (accessed via the existing `.command()` accessor, excluding `SkippedMissing` results), render `<a href="#cmd-{name}">(see: {command.title} ↓)</a>` instead of the raw string.
- `contrib/markdown.j2`:
  - Add `(see: {name})` after evidence signal IDs when `source_commands` is non-empty.
  - In suggest rendering: append ` (see above)` when suggest string exactly equals a run command's `.command()` value.
...
- Evidence command sections appear before non-evidence sections in HTML (in `command_results[0]` insertion order).
...
**C4** GIVEN `command_results[0]` contains evidence commands `["iostat", "sar_dev"]` and non-evidence commands `["df", "free"]` WHEN the HTML template renders THEN the `iostat` and `sar_dev` sections appear before the `<details>` block, and `df` and `free` appear inside `<details>`.
...
**C6** GIVEN `suggest = ["iostat -x 1 5"]` on a finding AND `command_results` contains a non-SkippedMissing command with `.command() = "iostat -x 1 5"` WHEN the HTML template renders the suggest list THEN the output contains `href="#cmd-iostat"` and does not contain the bare string `"iostat -x 1 5"` outside an anchor tag.
...
- `src/command.rs`:
  - Add `#[serde(skip_serializing_if = "Option::is_none")] pub(crate) use_dimension: Option<UseDimension>` to `Command`.
  - Add accessor `pub(crate) fn use_dimension(&self) -> Option<&UseDimension>`.
  - Add builder `pub(crate) fn with_use_dimension(mut self, v: UseDimension) -> Self`.
- `src/cli/config.rs`:
  - Add `#[serde(default)] pub followup: Vec<ProfileFollowup>` to `Profile`.
- `src/analysis.rs`:
  - Add `#[serde(skip)] pub(crate) vital_signs: VitalSigns` and `#[serde(skip)] pub(crate) use_coverage: Vec<UseCoverageEntry>` to `AnalysisReport`. Update all `AnalysisReport` construction sites (check `src/analysis.rs` — there is only one direct construction site; `with_*` setters or direct field assignment may also exist).
- `src/cli/mod.rs` (`generate_report`):
  - Compute `use_coverage`: for each of the 12 `(UseResource, UseAspect)` pairs, set `covered = true` if at least one command with that `use_dimension` returned a non-`SkippedMissing` result. Assign to `report.use_coverage`.
  - Compute `VitalSigns`:
    ...
    - Trend for CPU: `signal.stats.as_ref().map(|s| s.trend)` from the first `cpu.*` signal that has `stats`. No trend field on other resource lines.
    - When all `Option` fields for a resource line are `None`, the template renders `[not profiled]`.
  - Assign `report.vital_signs`.
  - Pass `report.vital_signs` and `report.use_coverage` to the template context (via the `AnalysisReport` already passed, or explicitly).
...
**C1** GIVEN `cpu.iowait_pct = 23.4` in signals AND a finding with `evidence[].signal_id = "cpu.iowait_pct"` and severity Warn WHEN `VitalSigns` is computed THEN `cpu.iowait_pct == 23.4` and `cpu.severity == Some(Warn)`.

**C2** GIVEN `VitalSigns.cpu.iowait_pct = Some(23.4)` and `cpu.severity = Some(Warn)` WHEN the report renders THEN the CPU vital signs line contains `"23"` and `"WARN"`.
...
**C4** GIVEN all network signal fields are `None` WHEN the report renders the Network vital signs line THEN the line contains `"[not profiled]"`.
...
**Dependencies:** none (can be implemented in parallel with Phases 3 and 4). Phase 2 owns the `run_diagnostics` signature change; this phase extends it further. Coordinate: Phase 2 changes `run_diagnostics` to `fn run_diagnostics(&self, command_results: &[CommandResult]) -> (Vec<Signal>, Vec<Finding>, Vec<String>)` — Phase 5 uses that signature as its starting point.

**Changes:**

- `src/command.rs`:
  - Add `#[serde(default)] pub(crate) extract: Vec<CommandExtract>` to `Command`.
  - Add accessor `pub(crate) fn extract(&self) -> &[CommandExtract]`.
- `src/cli/config.rs`:
  - Add `Error::InvalidExtractPattern { command: String, pattern: String, reason: String }` variant to the existing `cli/config` error type.
  - Extend `Config::validate` by calling a new private method `validate_extract_patterns(&self) -> Result<()>` from the existing method body. Do not change the call site at `cli/mod.rs:311`.
  - `validate_extract_patterns` iterates all commands; for each `extract` entry:
    1. Compile `extract.pattern` with `regex::Regex::new` — on error, return `Err(Error::InvalidExtractPattern { command: name, pattern, reason: err.to_string() })`.
    2. Unless `extract.aggregate == Aggregate::Count`, check that `extract.pattern` contains the substring `(?P<val>` — if not, return `Err(Error::InvalidExtractPattern { command: name, pattern, reason: "must contain a named group (?P<val>...)".to_string() })`.
  - Return the first error encountered; do not collect all errors.
- `src/extract.rs` (new file):
  ...
  - For each `CommandExtract`:
    - Compile `pattern` to a `Regex` (already validated; use `expect` with a message like `"extract pattern pre-validated"` ).
    - `Aggregate::Count`: count lines where pattern matches; always emit `Signal { id: extract.signal_id.clone(), value: SignalValue::I64(count as i64), unit: extract.unit, stats: None, at: chrono::Local::now() }` — emits `I64(0)` for 0 matches.
    - Other aggregates: for each matching line, extract the `val` named capture group, parse as `f64`. Skip non-numeric captures with `log::warn!("extract: non-numeric capture command={command_name} pattern={pattern} value={val}")`. Apply aggregate to the collected `f64` values:
      - `Last`: use the last value in the collected list.
      - `Max`: use the maximum value.
      - `Min`: use the minimum value.
      - `Avg`: sum / count (zero elements: emit nothing — no division).
    - If result set is empty (no matches, or all non-numeric, or Avg with zero), emit nothing and `log::warn!("extract: no matches for command={command_name} pattern={pattern}")`.
    - Non-Count signals: `Signal { id: extract.signal_id.clone(), value: SignalValue::F64(result), unit: extract.unit, stats: None, at: chrono::Local::now() }`.
- `src/analysis.rs` (`Analysis::run`):
  - After running commands and before running collectors, iterate `CommandResult::Success` results from the first repetition.
  ...
  - Pass the flattened first repetition as `command_results` to `run_diagnostics`.
- `src/cli/mod.rs`:
  - In `run_explain`: change signature to `fn run_explain(id: &str, config: &Config) -> miette::Result<()>`. Load config in `run_subcommand` using the same path resolution as `generate_report` (platform default when `--config` is absent); pass it to `run_explain`. When the config file is absent, pass an empty/default `Config` and fall back to rule/signal lookup.
  - In `run_explain`: before existing rule/signal lookup, check if `id` matches any command's `name` field in `config`. If found, render: `title`, `description`, `what_to_look_for` (if set), `extract` entries (pattern, signal_id, unit, aggregate each on a line), and `links`. If not found, fall through to existing rule/signal lookup unchanged.
- `contrib/linux.conf`:
...
**C1** GIVEN stdout `"Out of memory: Killed process 123\n"` and `extract = [{pattern: "Out of memory:", signal_id: "dmesg.oom_count", unit: "count", aggregate: "count"}]` WHEN `extract_signals` is called THEN it returns exactly one `Signal` with `id = "dmesg.oom_count"` and `SignalValue::I64(1)`.

**C2** GIVEN three stdout lines with `val` captures `"5"`, `"18"`, `"12"` and `aggregate = "max"` WHEN `extract_signals` is called THEN the returned signal has `SignalValue::F64(18.0)`.

**C3** GIVEN `aggregate = "count"` and a pattern that matches 3 lines WHEN `extract_signals` is called THEN the returned signal has `SignalValue::I64(3)`.

**C4** GIVEN three matching lines with `val` captures `"4"`, `"6"`, `"8"` and `aggregate = "avg"` WHEN `extract_signals` is called THEN the returned signal has `SignalValue::F64(6.0)`.

**C5** GIVEN three matching lines and `aggregate = "last"` WHEN `extract_signals` is called THEN the returned signal value equals `SignalValue::F64` of the last parsed capture.
...
**C10** GIVEN `vmstat` is a command in the loaded config with `title`, `description`, and `what_to_look_for` set WHEN `run_explain("vmstat", &config)` runs THEN the output contains all three field values.
...
| Walk `Predicate` tree for threshold index instead of parsing strings | Parse rule `when` as string | `Rule.when` is a `Predicate` enum (`Cmp`, `And`, `Or`) — no string to parse. Walk on `Predicate::Cmp { path, op, rhs: Rhs::Value(Value::Number(n)) }`; skip `And`, `Or`, non-numeric `Rhs`. |
...
| `VitalSigns` and `use_coverage` added to `AnalysisReport` with `#[serde(skip)]` | Computed only in `generate_report` locals | On `AnalysisReport` enables unit testing without going through the full render pipeline. `#[serde(skip)]` keeps `usereport diff` JSON backward-compatible. |
| `Config::validate` extended via private `validate_extract_patterns` helper | New `validate` function; changed call sites | `Config::validate` already exists at `src/cli/config.rs:134` and is called at `cli/mod.rs:311` — extending it preserves the existing call site unchanged. |
...
| `MemLine`, `DiskLine`, `NetLine` omit `trend` | All four lines have `trend` | Only CPU has trend in current collectors; adding unused `Option<Trend>` fields to other lines violates YAGNI. |
| `Severity::rank` kept as value receiver `pub fn rank(self) -> u8` | Change to `&self` | Existing callers use value receiver; changing to `&self` would break them. SDD originally stated `&self` — corrected to value receiver. |
| `run_explain` accepts `config: &Config` parameter | Load config inside `run_explain` | Consistent with existing pattern; caller (`run_subcommand`) already loads config for `generate_report`. |
| `SignalValue::I64(count)` for Count, `SignalValue::F64(result)` for other aggregates | Single variant | Count is always an integer; aggregations over parsed f64 are naturally f64. Matches existing `SignalValue` semantics. |
...
- `signal_ids()` method on the `Collector` trait: deferred; current hard-coded mapping in `generate_report` is sufficient.
- Error aggregation (returning all validation errors at once): first-error return is consistent with existing `Config::validate` behaviour.
